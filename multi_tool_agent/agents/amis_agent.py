...
class AmisAgent:
        ...
        try:
            ...
            for heteronym in heteronyms:
                ...
                for def_item in definitions:
                    ...
                    # 提取中文部分
                    chinese_def = self._extract_chinese_definition(def_text)
                    if chinese_def:
            ...
            # 處理同義詞（去重並清理格式）
            if all_synonyms:
                cleaned_synonyms = self._clean_synonyms(all_synonyms, word_list)
                if cleaned_synonyms:
            ...
            # 處理例句
            if all_examples:
                cleaned_examples = self._clean_examples(all_examples)
                if cleaned_examples:
    ...
    def _extract_chinese_definition(self, def_text):
        """提取定義中的中文部分"""
        if not def_text:
            return ""
    ...
    def _clean_synonyms(self, synonyms, word_list):
        """清理同義詞並驗證是否存在"""
        cleaned = []
        for synonym in synonyms:
            if synonym:
                ...
                # 檢查是否在詞典中存在
                if self._word_exists_in_dict(clean_word, word_list):
                    # 顯示原始格式
                    cleaned.append(synonym)
                else:
                    # 如果清理後的詞存在，顯示清理後的
                    if self._word_exists_in_dict(synonym, word_list):
                        cleaned.append(synonym)
                    else:
                        # 即使不存在也保留，可能是引用格式
                        cleaned.append(synonym)
    ...
    def _word_exists_in_dict(self, word, word_list):
        """檢查單字是否存在於詞典中"""
    ...
    def _clean_examples(self, examples):
        ...
        for example in examples:
            if example and example.strip():
                ...
                formatted_example = self._format_example(clean_example)
